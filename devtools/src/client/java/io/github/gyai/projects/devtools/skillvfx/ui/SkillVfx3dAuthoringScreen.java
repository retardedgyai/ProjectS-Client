...
public final class SkillVfx3dAuthoringScreen extends ProjectSThemedScreen {
    private static final String CAMERA_TIP = "Ctrl+ドラッグ: 視点 / 1〜4: Tool切替";
    private final SkillEditorScreen owner; private final AbilityVfx.Frame frame; private final SkillVfx3dAuthoringLifecycle lifecycle=new SkillVfx3dAuthoringLifecycle(); private SkillVfxDirectAuthoring.DragTransaction drag; private SkillVfxDirectAuthoring.Handle dragHandle,hover; private SkillVfxDirectAuthoring.Projection cameraProjection; private SkillVfxDirectAuthoring.MotionHandleTarget motionHandleTarget=SkillVfxDirectAuthoring.MotionHandleTarget.PHASE; private SkillVfxDirectAuthoring.AuthoringTool tool=SkillVfxDirectAuthoring.AuthoringTool.SHAPE; private String feedback=""; private boolean cameraDragging,returning,editZ;
    ...
    @Override protected void init(){
        ...
        var toolButton=button(x,y+48,Math.min(152,w),"Tool: "+tool.label(),()->{tool=tool.next();feedback=tool.label()+" Tool";clearWidgets();init();});
        toolButton.tooltip(Component.literal("3D編集Tool"),Component.literal("移動・回転・拡大縮小・形状を切り替えます。"),ProjectSTooltip.Tone.NORMAL);
        addRenderableWidget(toolButton);
        var edit=button(x,y+72,Math.min(materialize?152:96,w),materialize?"始点・終点を設定":editZ?"点編集: Z":"点編集: XY",materialize?this::materializeLine:()->{editZ=!editZ;clearWidgets();init();});
        edit.tooltip(Component.literal("直接操作"),Component.literal(CAMERA_TIP),ProjectSTooltip.Tone.NORMAL);
        addRenderableWidget(edit);
        var direction=button(x,y+96,Math.min(152,w),p==null?"方向":"方向: "+MotionAuthoringPresentation.label(p.motion().direction()),()->runAndRefresh(owner::toggleMotionDirection));
        ...
        direction.tooltip(Component.literal("軌道方向"),Component.literal("軌道をどちら向きに進むか指定します。"),ProjectSTooltip.Tone.NORMAL);
        ...
        if(motionControl.visible()){
            ...
            target.tooltip(Component.literal("モーション操作"),Component.literal(motionHandleTarget==SkillVfxDirectAuthoring.MotionHandleTarget.PHASE?"シアンの印をドラッグ":"オレンジの端をドラッグ"),ProjectSTooltip.Tone.NORMAL);
    ...
    @Override public void extractBackground(net.minecraft.client.gui.GuiGraphicsExtractor graphics,int mouseX,int mouseY,float tickProgress){ }
    ...
    @Override public void extractRenderState(net.minecraft.client.gui.GuiGraphicsExtractor graphics,int mouseX,int mouseY,float tickProgress){
        ...
        ProjectSUiDraw.cutPanel(graphics,overlay.bounds().x(),overlay.bounds().y(),overlay.bounds().width(),overlay.bounds().height(),8,0xD91E221F,t.borderCard());
        ProjectSTextRenderer.drawStrong(graphics,"VFX 3D DIRECT EDIT",overlay.title().x(),overlay.title().y()-1,10,t.accentPrimary());
        for(var chip:overlay.chips()){
            ...
            ProjectSTextRenderer.draw(graphics,chip.label(),chip.bounds().x()+16,chip.bounds().y()+3,8,t.textPrimary());
        }
        if(overlay.directVisible()){
            ...
            ProjectSTextRenderer.drawStrong(graphics,motionHandleTarget==SkillVfxDirectAuthoring.MotionHandleTarget.TRAIL?"直接操作: 軌跡":"直接操作: 開始位置",overlay.directOperation().x()+16,overlay.directOperation().y()+4,8,directColor);
        }
        String context=hover==null?feedback:SkillVfxVisualUxPresentation.hoverContext(hover);
        if(!context.isBlank())ProjectSTextRenderer.draw(graphics,context,overlay.feedback().x(),overlay.feedback().y(),8,t.textSecondary());
    ...
    @Override public boolean mouseClicked(MouseButtonEvent event,boolean doubleClick){if(super.mouseClicked(event,doubleClick))return true;if(event.button()!=0)return true;if(event.hasControlDown()){cameraDragging=true;feedback="ゲーム内視点を操作中";return true;}var p=owner.authoringPrimitive();var d=owner.worldPreviewDocument();if(p==null||d==null)return true;var hit=hit(p,event.x(),event.y());if(hit.isPresent()){dragHandle=hit.get().handle();drag=SkillVfxDirectAuthoring.DragTransaction.begin(d,p.id()).orElse(null);feedback=dragHandle.label();return true;}var picked=SkillVfxDirectAuthoring.pickPrimitive(owner.authoringTargets(),frame,cameraProjection,event.x()-width/2.0,height/2.0-event.y(),8);if(picked.isPresent()){owner.selectAuthoringTarget(picked.get().target());feedback=SkillVfxDisplay.primitive(picked.get().target().primitive().type())+"を選択";clearWidgets();init();}return true;}
    ...
    @Override public boolean keyPressed(KeyEvent event){if(event.key()==GLFW.GLFW_KEY_ESCAPE&&drag!=null){drag.cancel();drag=null;dragHandle=null;owner.authoringPreviewChanged(frame);feedback="キャンセル";return true;}if(super.keyPressed(event))return true;var selected=switch(event.key()){case GLFW.GLFW_KEY_1->SkillVfxDirectAuthoring.AuthoringTool.MOVE;case GLFW.GLFW_KEY_2->SkillVfxDirectAuthoring.AuthoringTool.ROTATE;case GLFW.GLFW_KEY_3->SkillVfxDirectAuthoring.AuthoringTool.SCALE;case GLFW.GLFW_KEY_4->SkillVfxDirectAuthoring.AuthoringTool.SHAPE;default->null;};if(selected!=null){tool=selected;feedback=tool.label()+" Tool";clearWidgets();init();}return true;}
    @Override public boolean keyReleased(KeyEvent event){return super.keyReleased(event);}
    ...
    public SkillVfxDirectAuthoring.AuthoringTool authoringTool(){return tool;}
    private Optional<SkillVfxDirectAuthoring.Pick> hit(SkillVfxModel.Primitive p,double x,double y){if(cameraProjection==null)return Optional.empty();return SkillVfxDirectAuthoring.pick(SkillVfxDirectAuthoring.toolHandles(p,frame,owner.authoringAction(),owner.authoringProgress(),motionHandleTarget(),tool),cameraProjection,x-width/2.0,height/2.0-y,12);}
}
