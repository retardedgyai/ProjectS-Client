...
public final class BalanceClientState {
    ...
    public static void receive(BalanceStatePayload.State updated) {
        ...
        localRevision++;
        SkillDescriptionStore.publish(descriptionSnapshot());
    ...
    /** DevTools maps received values over Core's read-only player description snapshot. */
    public static SkillDescriptionSnapshot descriptionSnapshot() {
        SkillDescriptionSnapshot base = SkillDescriptionSnapshot.fromCatalog();
        java.util.Map<String, String> mapped = new java.util.LinkedHashMap<>(base.values());
        for (SkillCatalog.Group group : SkillCatalog.GROUPS) for (SkillCatalog.Skill skill : group.skills()) mapped.put(skill.id(), skillDescription(skill));
        return new SkillDescriptionSnapshotAccessor(mapped).snapshot();
    }

    private record SkillDescriptionSnapshotAccessor(java.util.Map<String, String> values) {
        private SkillDescriptionSnapshot snapshot() { return SkillDescriptionSnapshot.fromValues(values); }
    }
}
