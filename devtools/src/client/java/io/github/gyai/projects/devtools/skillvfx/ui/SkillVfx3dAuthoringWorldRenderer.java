...
public final class SkillVfx3dAuthoringWorldRenderer {
    ...
    private static void render(LevelRenderContext context) {
        try {
            ...
            for (AbilityVfx.Command command : SkillVfxDirectAuthoring.guide(primitive, frame, action)) {
                commands.add(new AbilityVfx.Command(command.a(), command.b(),
                        new AbilityVfx.Color(75, 220, 255, 150), Math.max(.012, command.width())));
            }
            double progress = AbilityVfxLocalPreview.preview()
                    .map(p -> Math.clamp((p.tick() - primitive.delayTicks())
                            / (double) Math.max(1, primitive.durationTicks()), 0, 1))
                    .orElse(0d);
            for (AbilityVfx.Command command : SkillVfxDirectAuthoring.guide(
                    primitive, frame, action, progress)) {
                commands.add(new AbilityVfx.Command(command.a(), command.b(),
                        new AbilityVfx.Color(255, 235, 80, 235), Math.max(.02, command.width())));
            }
            ...
            List<SkillVfxDirectAuthoring.Handle> handles = SkillVfxDirectAuthoring.worldHandles(primitive, frame);
