...
public final class SkillEditorScreen extends ProjectSThemedScreen {
    ...
    private SkillVfxDirectAuthoring.Projection embeddedCameraProjection;
    private SkillVfxDirectAuthoring.DragTransaction embeddedDrag;
    private SkillVfxDirectAuthoring.Handle embeddedDragHandle,embeddedHover;
    private SkillVfxDirectAuthoring.AuthoringTool embeddedTool=SkillVfxDirectAuthoring.AuthoringTool.SHAPE;
    private SkillVfxDirectAuthoring.DragTransaction timelineDrag;
    private TimelineDragMode timelineDragMode;
    private double timelineDragStartX;
    private int timelineDragStartTick, timelineDragEndTick, timelineDragDuration;
    private enum TimelineDragMode { MOVE, RESIZE_START, RESIZE_END }
    private boolean embeddedCameraDragging;
    ...
    @Override protected void rebuildWidgets(){
        clearWidgets(); labels.clear(); glyphs.clear(); pickerCards.clear(); var doc=SkillEditorClientState.document();
    ...
    private void buildPicker(AbilityVisualEditorDocument doc){
        String title=picker==Picker.PRIMITIVE?(pickerCreatesEmission?"演出の最初のレイヤーを選択":"レイヤーを追加"):"見た目を選択";
        List<SkillVfxVisualUxPresentation.CardItem> items=picker==Picker.PRIMITIVE?SkillVfxVisualUxPresentation.primitiveCards():SkillVfxVisualUxPresentation.appearanceCards();
        var page=SkillVfxVisualUxPresentation.pickerPage(width,height,title,items,pickerPage);pickerPage=page.page();drawLater(page.title(),page.bounds().x(),page.bounds().y());
        for(var card:page.cards()){
            pickerCards.add(card);
            if(picker==Picker.PRIMITIVE){var type=SkillVfxModel.PrimitiveType.valueOf(card.item().id());var control=button(card.action().x(),card.action().y(),card.action().width(),"選択",()->choosePrimitive(type));control.tooltip(Component.literal(card.item().label()),Component.literal(card.item().tooltip()),ProjectSTooltip.Tone.NORMAL);add(control);}
            else{var choice=SkillVfxAuthoring.appearances().stream().filter(value->value.appearance().id().equals(card.item().id())).findFirst().orElseThrow();var control=button(card.action().x(),card.action().y(),card.action().width(),"選択",()->chooseAppearance(choice.appearance()));control.tooltip(Component.literal(card.item().label()),Component.literal(card.item().tooltip()),ProjectSTooltip.Tone.NORMAL);add(control);}
            glyphs.add(new GlyphPlacement(card.item().glyph(),card.preview().x(),card.preview().y(),card.preview().width()));
            drawLater(card.item().label(),card.bounds().x()+56,card.bounds().y()+8);
            drawLater(card.item().detail(),card.bounds().x()+56,card.bounds().y()+26);
        }
    ...
    private final List<GlyphPlacement> glyphs=new ArrayList<>();
    private final List<SkillVfxVisualUxPresentation.PickerCard> pickerCards=new ArrayList<>();
    private record Label(String text,int x,int y) { }
    private record GlyphPlacement(SkillVfxVisualUxPresentation.Glyph glyph,int x,int y,int size) { }
    private void drawLater(String text,int x,int y){labels.add(new Label(text,x,y));}
    private void drawGlyphLater(SkillVfxVisualUxPresentation.Glyph glyph,int x,int y,int size){if(glyph!=null)glyphs.add(new GlyphPlacement(glyph,x,y,Math.max(8,size)));}
    private void buildTree(AbilityVisualEditorDocument doc,SkillEditorLayout.Rect r){
        if(r.width()<=0||r.height()<=0)return;
        int x=r.x()+6,w=Math.max(70,r.width()-12),small=Math.max(30,(w-8)/2),top=r.y()+28;
        add(button(x,top,small,treeActions?"一覧へ":"レイヤー編集",()->{treeActions=!treeActions;rebuildWidgets();}));
    ...
    private void buildTreeActions(AbilityVisualEditorDocument doc,int x,int y,int column){
        ...
        add(button(x,y,column,"+ 新しい演出",this::addEmission));
        ...
        var add=button(x,y+22,column,"+ レイヤー",this::addPrimitive);add.active=selected!=null;add(add);var duplicate=button(x+column+4,y+22,column,"複製",this::duplicatePrimitive);duplicate.active=primitive!=null;add(duplicate);
        var remove=button(x,y+44,column,"レイヤー削除",this::removePrimitive);remove.active=primitive!=null;add(remove);var removeEmission=button(x+column+4,y+44,column,"演出を削除",this::removeEmission);removeEmission.active=selected!=null;add(removeEmission);
        ...
        var emissionUp=button(x,y+88,column,"演出を上へ",()->moveEmission(-1));emissionUp.active=canMoveEmission(doc,-1);add(emissionUp);var emissionDown=button(x+column+4,y+88,column,"演出を下へ",()->moveEmission(1));emissionDown.active=canMoveEmission(doc,1);add(emissionDown);
        var duplicateEmission=button(x,y+110,column,"演出を複製",this::duplicateEmission);duplicateEmission.active=selected!=null;add(duplicateEmission);
    }
    private void buildPreview(AbilityVisualEditorDocument doc,SkillEditorLayout.Rect r){
        if(r.width()<=0||r.height()<=0)return;var p=primitive(doc);ensureEmbeddedPreview(p);var page=layout.previewPage(r,previewDetails,SkillVfxDisplay.hook(hook),p==null?"":SkillVfxDisplay.primitive(p.type()),preview.anchor()==SkillVfxPreviewController.Anchor.PLAYER?"プレイヤー周囲":"プレイヤー前方3m",SkillEditorClientState.previewAllowed(),localError,font::width);
    ...
    private void buildInspector(AbilityVisualEditorDocument doc,SkillEditorLayout.Rect r){
        ...
        drawGlyphLater(SkillVfxVisualUxPresentation.shapeGlyph(p.type()),header.title().x(),header.title().y(),12);drawLater(layout.inspectorTitle(p.type(),first,last,properties.size(),r),header.title().x()+16,header.title().y());drawLater(layout.inspectorPageLabel(first,last,properties.size()),header.page().x(),header.page().y()+5);
        ...
        for(var entry:page.entries()){
            ...
            if(field.equals("motionCategory")){drawInspectorLabel(field,MotionAuthoringPresentation.categoryLabel(),p,row.label());continue;}
            drawInspectorLabel(field,fieldLabel,p,row.label());
    ...
    private void buildMotionField(AbilityVisualEditorDocument doc,SkillVfxModel.Primitive primitive,String field,SkillEditorLayout.Rect input,SkillEditorLayout.Rect help){
        ...
        switch(field){
            case "motionMode"->{var choices=MotionAuthoringPresentation.modes(primitive.type(),primitive.motion());var control=button(x,y,w,MotionAuthoringPresentation.label(primitive.motion().mode()),()->cycleMotionMode(primitive));control.active=choices.stream().anyMatch(value->value.enabled()&&value.value()!=primitive.motion().mode());control.tooltip(Component.literal("モーション"),Component.literal(MotionAuthoringPresentation.modeTooltip(primitive.type(),primitive.motion())),ProjectSTooltip.Tone.NORMAL);add(control);}
            case "motionDirection"->{var control=button(x,y,w,MotionAuthoringPresentation.label(primitive.motion().direction()),()->cycleMotionDirection(primitive));control.active=MotionAuthoringPresentation.canToggleDirection(primitive.type(),primitive.motion());control.tooltip(Component.literal("方向"),Component.literal(tooltip),ProjectSTooltip.Tone.NORMAL);add(control);}
            case "motionPhase"->{var number=new ProjectSNumberField(font,x,y,w,18,Component.empty(),primitive.motion().phase()*100,0,100,1,0,"%",value->setMotion(primitive.id(),current->MotionAuthoringPresentation.phase(current,primitive.type(),value/100d),false));number.active=primitive.motion().mode()!=io.github.gyai.projects.client.vfx.MotionMode.STATIC;number.tooltip(Component.literal("位相"),Component.literal(tooltip),ProjectSTooltip.Tone.NORMAL);add(number);}
            case "motionEasing"->{var control=button(x,y,w,MotionAuthoringPresentation.label(primitive.motion().easing()),()->cycleMotionEasing(primitive));control.active=MotionAuthoringPresentation.easings(primitive.type(),primitive.motion()).stream().anyMatch(value->value.enabled()&&value.value()!=primitive.motion().easing());control.tooltip(Component.literal("イージング"),Component.literal(tooltip),ProjectSTooltip.Tone.NORMAL);add(control);}
            case "motionTrail"->{var number=new ProjectSNumberField(font,x,y,w,18,Component.empty(),primitive.motion().trailFraction()*100,0,100,1,0,"%",value->setMotion(primitive.id(),current->MotionAuthoringPresentation.trail(current,primitive.type(),value/100d),false));number.tooltip(Component.literal("軌跡"),Component.literal(tooltip),ProjectSTooltip.Tone.NORMAL);add(number);}
            default->{ }
        }
    ...
    private void drawInspectorLabel(String field,String label,SkillVfxModel.Primitive primitive,SkillEditorLayout.Rect bounds){var glyph=SkillVfxVisualUxPresentation.inspectorGlyph(field,primitive);if(glyph==null){drawWrapped(label,bounds);return;}drawGlyphLater(glyph,bounds.x(),bounds.y(),12);drawWrapped(label,new SkillEditorLayout.Rect(bounds.x()+16,bounds.y(),Math.max(1,bounds.width()-16),bounds.height()));}
    ...
    @SuppressWarnings({"rawtypes","unchecked"}) private void writeDescriptor(io.github.gyai.projects.editor.core.PropertyDescriptor descriptor,AbilityVisualEditorDocument document,Object value){descriptor.write(document,value);afterMutation(true);}
    ...
    private void setControlPoints(AbilityVisualEditorDocument document,String id,java.util.function.Function<SkillVfxModel.Primitive,List<SkillVfxModel.Vec>> update,boolean rebuild){if(SkillVfxMutation.writeControls(document,id,update))afterMutation(true);}
    private void setPrimitive(String id,java.util.function.UnaryOperator<SkillVfxModel.Primitive> update,boolean rebuild){var d=document();if(SkillVfxMutation.replace(d,id,update)){localError="";afterMutation(true);}}
    ...
    private void removeEmission(){var d=document();if(d==null||emissionId==null)return;String removed=emissionId;var selected=emission(d);int count=selected==null?0:selected.primitives().size();modal.open(Component.literal("演出を削除しますか？"),Component.literal("中のレイヤー "+count+" 個も削除します。"),Component.literal("削除"),()->{d.execute(AbilityVisualCommands.removeEmission(hook,removed));emissionId=null;primitiveId=null;afterMutation(true);},Component.literal("キャンセル"),()->{},ProjectSModal.PrimaryKind.DANGER,true);}
    ...
    @Override public void extractBackground(GuiGraphicsExtractor graphics,int mouseX,int mouseY,float tickProgress){ }
    @Override public void extractRenderState(GuiGraphicsExtractor graphics,int mouseX,int mouseY,float tickProgress){var theme=ProjectSThemeManager.get().activeTheme();var t=theme.tokens();var d=document();var bounds=layout.bounds(width,height);ProjectSUiDraw.panelShadow(graphics,bounds.header().x(),bounds.header().y(),bounds.header().width(),bounds.header().height(),10,t.shadow());ProjectSUiDraw.cutPanel(graphics,bounds.header().x(),bounds.header().y(),bounds.header().width(),bounds.header().height(),theme.metrics().cornerCut(),t.surface(),t.borderCard());int statusX=width<760?240:166,statusY=width<760?10:31;ProjectSTextRenderer.drawMono(graphics,d!=null&&d.dirty()?"● UNSAVED CHANGES":"SYNCHRONIZED",statusX,statusY,7,d!=null&&d.dirty()?t.warning():t.textMuted());if(!displaySettings&&picker==Picker.NONE){panel(graphics,bounds.tree(),"1  いつ出す？ / レイヤー");viewportPanel(graphics,bounds.preview(),"2  形を確認");panel(graphics,bounds.inspector(),"3  形・見た目・動き");panel(graphics,bounds.timeline(),"4  開始と表示時間");drawViewportOverlay(graphics,d,bounds.preview(),t);drawTimeline(graphics,d,bounds.timeline(),t);}else{graphics.fill(0,46,width,height,t.background());if(displaySettings)ProjectSTextRenderer.drawStrong(graphics,"表示設定",8,48,11,t.textPrimary());}if(picker!=Picker.NONE)for(var card:pickerCards){var box=card.bounds();ProjectSUiDraw.cutPanel(graphics,box.x(),box.y(),box.width(),box.height(),theme.metrics().cornerCut(),t.surfaceRaised(),t.borderCard());}for(var placement:glyphs)drawGlyph(graphics,placement,t);super.extractRenderState(graphics,mouseX,mouseY,tickProgress);for(var label:labels)ProjectSTextRenderer.draw(graphics,label.text(),label.x(),label.y(),8,t.textSecondary());}
    private void viewportPanel(GuiGraphicsExtractor graphics,SkillEditorLayout.Rect r,String title){if(r.width()<=0||r.height()<=0)return;var t=ProjectSThemeManager.get().activeTheme().tokens();graphics.fill(r.x(),r.y(),r.x()+r.width(),r.y()+26,0xE6292D29);graphics.outline(r.x(),r.y(),r.width(),r.height(),t.borderCard());ProjectSTextRenderer.drawMono(graphics,title,r.x()+10,r.y()+9,8,t.accentPrimary());}
    private void drawViewportOverlay(GuiGraphicsExtractor graphics,AbilityVisualEditorDocument doc,SkillEditorLayout.Rect r,io.github.gyai.projects.client.ui.theme.ProjectSThemeTokens t){if(r.width()<=0||r.height()<=0)return;int left=r.x()+8,right=r.x()+r.width()-8,bottom=r.y()+r.height()-8;ProjectSTextRenderer.drawMono(graphics,"CTRL + DRAG  CAMERA",right-116,r.y()+9,7,t.textMuted());var selected=primitive(doc);if(selected!=null){ProjectSTextRenderer.drawStrong(graphics,SkillVfxDisplay.hook(hook)+"  >  "+SkillVfxDisplay.primitive(selected.type()),left,bottom-17,9,t.textPrimary());ProjectSTextRenderer.drawMono(graphics,"形状をクリックで選択 / Handleをドラッグ",left,bottom-7,7,t.textMuted());}else ProjectSTextRenderer.draw(graphics,"左で表示タイミングを選び、レイヤーを追加",left,r.y()+42,8,t.textMuted());if(embeddedHover!=null)ProjectSTextRenderer.drawStrong(graphics,SkillVfxVisualUxPresentation.hoverContext(embeddedHover),left,r.y()+70,8,t.accentPrimary());}
    private void drawGlyph(GuiGraphicsExtractor graphics,GlyphPlacement placement,io.github.gyai.projects.client.ui.theme.ProjectSThemeTokens tokens){drawGlyph(graphics,placement.glyph(),tokens,placement.x(),placement.y(),placement.size());}
     private void drawGlyph(GuiGraphicsExtractor graphics,SkillVfxVisualUxPresentation.Glyph glyph,io.github.gyai.projects.client.ui.theme.ProjectSThemeTokens tokens,int x,int y,int size){int color=switch(glyph.kind()){case APPEARANCE->glyph.color();case TRAIL->tokens.warning();case PHASE->tokens.accentCyan();case DIRECTION->tokens.textPrimary();case EASING->tokens.success();case MOTION->tokens.accentPrimary();default->tokens.accentCyan();};for(var mark:glyph.marks()){if(mark instanceof SkillVfxVisualUxPresentation.Mark.Segment s)glyphLine(graphics,x,y,size,s.x1(),s.y1(),s.x2(),s.y2(),color);else if(mark instanceof SkillVfxVisualUxPresentation.Mark.Dot d){int cx=x+(int)Math.round(d.x()*(size-1)),cy=y+(int)Math.round(d.y()*(size-1)),radius=Math.max(1,(int)Math.round(d.radius()*size));if(d.filled())graphics.fill(cx-radius,cy-radius,cx+radius+1,cy+radius+1,color);else graphics.outline(cx-radius,cy-radius,Math.max(2,radius*2),Math.max(2,radius*2),color);}else if(mark instanceof SkillVfxVisualUxPresentation.Mark.Box b){int left=x+(int)Math.round(b.left()*(size-1)),top=y+(int)Math.round(b.top()*(size-1)),right=x+(int)Math.round(b.right()*(size-1)),bottom=y+(int)Math.round(b.bottom()*(size-1));if(b.filled())graphics.fill(left,top,right+1,bottom+1,color);else{graphics.horizontalLine(left,right,top,color);graphics.horizontalLine(left,right,bottom,color);graphics.verticalLine(left,top,bottom,color);graphics.verticalLine(right,top,bottom,color);}}else if(mark instanceof SkillVfxVisualUxPresentation.Mark.Diamond d){int cx=x+(int)Math.round(d.x()*(size-1)),cy=y+(int)Math.round(d.y()*(size-1)),radius=Math.max(1,(int)Math.round(d.radius()*size));glyphLine(graphics,x,y,size,(cx-x)/(double)Math.max(1,size-1),(cy-radius-y)/(double)Math.max(1,size-1),(cx+radius-x)/(double)Math.max(1,size-1),(cy-y)/(double)Math.max(1,size-1),color);glyphLine(graphics,x,y,size,(cx+radius-x)/(double)Math.max(1,size-1),(cy-y)/(double)Math.max(1,size-1),(cx-x)/(double)Math.max(1,size-1),(cy+radius-y)/(double)Math.max(1,size-1),color);glyphLine(graphics,x,y,size,(cx-x)/(double)Math.max(1,size-1),(cy+radius-y)/(double)Math.max(1,size-1),(cx-radius-x)/(double)Math.max(1,size-1),(cy-y)/(double)Math.max(1,size-1),color);glyphLine(graphics,x,y,size,(cx-radius-x)/(double)Math.max(1,size-1),(cy-y)/(double)Math.max(1,size-1),(cx-x)/(double)Math.max(1,size-1),(cy-radius-y)/(double)Math.max(1,size-1),color);}else if(mark instanceof SkillVfxVisualUxPresentation.Mark.Arc a){int steps=Math.max(8,(int)Math.ceil(Math.abs(a.sweepDegrees())/18));double previous=Math.toRadians(a.startDegrees());double px=a.centerX()+Math.cos(previous)*a.radius(),py=a.centerY()+Math.sin(previous)*a.radius();for(int step=1;step<=steps;step++){double angle=Math.toRadians(a.startDegrees()+a.sweepDegrees()*step/steps),nx=a.centerX()+Math.cos(angle)*a.radius(),ny=a.centerY()+Math.sin(angle)*a.radius();glyphLine(graphics,x,y,size,px,py,nx,ny,color);px=nx;py=ny;}}}}
     private void glyphLine(GuiGraphicsExtractor graphics,int x,int y,int size,double x1,double y1,double x2,double y2,int color){int steps=Math.max(1,(int)Math.ceil(Math.hypot(x2-x1,y2-y1)*size*1.5));for(int step=0;step<=steps;step++){double p=step/(double)steps;int px=x+(int)Math.round((x1+(x2-x1)*p)*(size-1)),py=y+(int)Math.round((y1+(y2-y1)*p)*(size-1));graphics.fill(px,py,px+2,py+2,color);}}
    private void drawTimeline(GuiGraphicsExtractor graphics,AbilityVisualEditorDocument doc,SkillEditorLayout.Rect r,io.github.gyai.projects.client.ui.theme.ProjectSThemeTokens t){
        ...
        var view=SkillVfxTimelinePresentation.layout(bars,duration,preview.timeline().ticks(),timelineViewport(r));
        for(var bar:view.bars()){boolean selected=primitiveId!=null&&bar.id().endsWith("/"+primitiveId);graphics.fill(bar.bounds().x(),bar.bounds().y(),bar.bounds().x()+bar.bounds().width(),bar.bounds().y()+bar.bounds().height(),selected?t.accentPrimaryHover():t.accentPrimaryPressed());}
     ...
     private boolean beginTimelineDrag(MouseButtonEvent event,SkillVfxTimelinePresentation.Rect viewport){
         var d=document();if(d==null||event.x()<viewport.x()||event.x()>=viewport.x()+viewport.width()||event.y()<viewport.y()||event.y()>=viewport.y()+viewport.height())return false;
         int duration=Math.max(1,preview.timeline().bars(d.visual(),hook).stream().mapToInt(SkillVfxTimeline.Bar::end).max().orElse(20));
         var view=SkillVfxTimelinePresentation.layout(preview.timeline().bars(d.visual(),hook),duration,preview.timeline().ticks(),viewport);
         for(var bar:view.bars())if(event.x()>=bar.bounds().x()&&event.x()<bar.bounds().x()+bar.bounds().width()&&event.y()>=bar.bounds().y()&&event.y()<bar.bounds().y()+bar.bounds().height()){
             int slash=bar.id().indexOf('/');if(slash<=0||slash>=bar.id().length()-1)continue;
             emissionId=bar.id().substring(0,slash);primitiveId=bar.id().substring(slash+1);d.select("primitive",primitiveId);
             var p=primitive(d);if(p==null)return true;
             int edge=Math.min(6,Math.max(2,bar.bounds().width()/4));
             timelineDragMode=event.x()-bar.bounds().x()<edge?TimelineDragMode.RESIZE_START:event.x()+edge>=bar.bounds().x()+bar.bounds().width()?TimelineDragMode.RESIZE_END:TimelineDragMode.MOVE;
             timelineDrag=SkillVfxDirectAuthoring.DragTransaction.begin(d,primitiveId).orElse(null);timelineDragStartX=event.x();timelineDragStartTick=p.delayTicks();timelineDragEndTick=p.delayTicks()+p.durationTicks();timelineDragDuration=duration;
             if(timelineDrag!=null)return true;
         }
         if(primitiveId!=null){
             for(var bar:view.bars())if(bar.id().endsWith("/"+primitiveId)){
                 var p=primitive(d);if(p==null)return false;
                 emissionId=bar.id().substring(0,bar.id().indexOf('/'));timelineDragMode=TimelineDragMode.MOVE;timelineDrag=SkillVfxDirectAuthoring.DragTransaction.begin(d,primitiveId).orElse(null);timelineDragStartX=event.x();timelineDragStartTick=p.delayTicks();timelineDragEndTick=p.delayTicks()+p.durationTicks();timelineDragDuration=duration;
                 return timelineDrag!=null;
             }
         }
         return false;
     }
     private void updateTimelineDrag(double mouseX){
         if(timelineDrag==null)return;int delta=(int)Math.round((mouseX-timelineDragStartX)*timelineDragDuration/(double)Math.max(1,timelineViewport(layout.bounds(width,height).timeline()).width()));
         int start=timelineDragStartTick,end=timelineDragEndTick;switch(timelineDragMode){case MOVE->{int length=end-start;start=Math.max(0,start+delta);end=start+length;}case RESIZE_START->start=Math.clamp(start+delta,0,end-1);case RESIZE_END->end=Math.max(start+1,end+delta);}
         int nextStart=start,nextDuration=Math.clamp(end-start,1,1200);timelineDrag.update(p->withNumber(withNumber(p,"delayTicks",nextStart),"durationTicks",nextDuration));preview.timeline().seek(Math.max(0,preview.timeline().ticks()));syncLocalPreview();rebuildWidgets();
     }
    private void panel(GuiGraphicsExtractor graphics,SkillEditorLayout.Rect r,String title){if(r.width()<=0||r.height()<=0)return;var theme=ProjectSThemeManager.get().activeTheme();var t=theme.tokens();ProjectSUiDraw.panelShadow(graphics,r.x(),r.y(),r.width(),r.height(),10,t.shadow());ProjectSUiDraw.cutPanel(graphics,r.x(),r.y(),r.width(),r.height(),theme.metrics().cornerCut(),t.surface(),t.borderCard());ProjectSTextRenderer.drawMono(graphics,title,r.x()+10,r.y()+9,8,t.accentPrimary());}
    @Override public boolean mouseClicked(MouseButtonEvent event,boolean doubleClick){if(super.mouseClicked(event,doubleClick))return true;var b=layout.bounds(width,height);if(event.button()==0){if(insideEmbeddedViewport(event.x(),event.y())){if(event.hasControlDown()){embeddedCameraDragging=true;return true;}var hit=embeddedHit(event.x(),event.y());if(hit.isPresent()){embeddedDragHandle=hit.get().handle();embeddedDrag=SkillVfxDirectAuthoring.DragTransaction.begin(document(),primitiveId).orElse(null);return true;}var picked=SkillVfxDirectAuthoring.pickPrimitive(authoringTargets(),authoringFrame,embeddedCameraProjection,event.x()-width/2.0,height/2.0-event.y(),8);if(picked.isPresent()){selectAuthoringTarget(picked.get().target());rebuildWidgets();}return true;}if(Math.abs(event.x()-(b.tree().x()+b.tree().width()))<5&&b.tree().width()>0){draggingLeft=true;return true;}if(Math.abs(event.x()-b.inspector().x())<5&&b.inspector().width()>0){draggingRight=true;return true;}if(Math.abs(event.y()-b.timeline().y())<5&&b.timeline().height()>0){draggingBottom=true;return true;}var viewport=timelineViewport(b.timeline());if(event.x()>=viewport.x()&&event.x()<viewport.x()+viewport.width()&&event.y()>=viewport.y()&&event.y()<viewport.y()+viewport.height()){var d=document();int duration=d==null?20:Math.max(1,preview.timeline().bars(d.visual(),hook).stream().mapToInt(SkillVfxTimeline.Bar::end).max().orElse(20));double seek=SkillVfxTimelinePresentation.seekTick(event.x(),duration,viewport);preview.timeline().seek(seek);AbilityVfxLocalPreview.seek(seek);if(d!=null){var view=SkillVfxTimelinePresentation.layout(preview.timeline().bars(d.visual(),hook),duration,preview.timeline().ticks(),viewport);for(var bar:view.bars()){var r=bar.bounds();if(event.x()>=r.x()&&event.x()<r.x()+r.width()&&event.y()>=r.y()&&event.y()<r.y()+r.height()){int slash=bar.id().indexOf('/');if(slash>0&&slash<bar.id().length()-1){emissionId=bar.id().substring(0,slash);primitiveId=bar.id().substring(slash+1);d.select("primitive",primitiveId);rebuildWidgets();}break;}}}return true;}}return false;}
      @Override public boolean mouseDragged(MouseButtonEvent event,double deltaX,double deltaY){if(embeddedCameraDragging){if(minecraft.player!=null)minecraft.player.turn((float)deltaX,(float)deltaY);return true;}if(embeddedDrag!=null&&embeddedDragHandle!=null&&embeddedCameraProjection!=null){if(embeddedDragHandle.kind()==SkillVfxDirectAuthoring.Handle.Kind.PHASE||embeddedDragHandle.kind()==SkillVfxDirectAuthoring.Handle.Kind.TRAIL)embeddedDrag.update(p->SkillVfxDirectAuthoring.editAtMouse(p,embeddedDragHandle,authoringFrame,authoringAction(),authoringProgress(),embeddedCameraProjection,event.x()-width/2.0,height/2.0-event.y()));else{var delta=SkillVfxDirectAuthoring.dragDelta(embeddedDrag.current(),embeddedDragHandle,authoringFrame,embeddedCameraProjection,deltaX,-deltaY,false);embeddedDrag.update(p->SkillVfxDirectAuthoring.edit(p,embeddedDragHandle,delta.x(),delta.y(),false));}authoringPreviewChanged(authoringFrame);return true;}if(timelineDrag==null){var timeline=layout.bounds(width,height).timeline();if(beginTimelineDrag(event,timelineViewport(timeline)))return true;}if(timelineDrag!=null){updateTimelineDrag(event.x());return true;}if(super.mouseDragged(event,deltaX,deltaY))return true;if(draggingLeft)layout.dragLeft((int)event.x(),width);else if(draggingRight)layout.dragRight((int)event.x(),width);else if(draggingBottom)layout.dragBottom((int)event.y(),height);else return false;rebuildWidgets();return true;}
     @Override public boolean mouseReleased(MouseButtonEvent event){if(event.button()==0&&embeddedCameraDragging){embeddedCameraDragging=false;return true;}if(event.button()==0&&embeddedDrag!=null){embeddedDrag.release();embeddedDrag=null;embeddedDragHandle=null;afterMutation(true);return true;}if(event.button()==0&&timelineDrag!=null){timelineDrag.release();timelineDrag=null;timelineDragMode=null;afterMutation(true);return true;}if(draggingLeft||draggingRight||draggingBottom){draggingLeft=draggingRight=draggingBottom=false;return true;}return super.mouseReleased(event);}
    @Override public void mouseMoved(double x,double y){embeddedHover=insideEmbeddedViewport(x,y)?embeddedHit(x,y).map(SkillVfxDirectAuthoring.Pick::handle).orElse(null):null;super.mouseMoved(x,y);}
    /** Screen-local only: text, modal and dropdown input get first refusal from the themed base. */
    @Override public boolean keyPressed(KeyEvent event){if(event.key()==GLFW.GLFW_KEY_ESCAPE&&embeddedDrag!=null){embeddedDrag.cancel();embeddedDrag=null;embeddedDragHandle=null;authoringPreviewChanged(authoringFrame);return true;}if(super.keyPressed(event))return true;if(getFocused() instanceof net.minecraft.client.gui.components.EditBox)return false;var selectedTool=toolForKey(event.key());if(selectedTool!=null){embeddedTool=selectedTool;rebuildWidgets();return true;}if(!event.hasControlDown())return false;var d=document();if(d==null)return false;boolean changed=switch(event.key()){case GLFW.GLFW_KEY_Z->event.hasShiftDown()?d.redo():d.undo();case GLFW.GLFW_KEY_Y->d.redo();default->false;};if(changed)afterMutation(true);return changed;}
    private static SkillVfxDirectAuthoring.AuthoringTool toolForKey(int key){return switch(key){case GLFW.GLFW_KEY_1->SkillVfxDirectAuthoring.AuthoringTool.MOVE;case GLFW.GLFW_KEY_2->SkillVfxDirectAuthoring.AuthoringTool.ROTATE;case GLFW.GLFW_KEY_3->SkillVfxDirectAuthoring.AuthoringTool.SCALE;case GLFW.GLFW_KEY_4->SkillVfxDirectAuthoring.AuthoringTool.SHAPE;default->null;};}
    ...
    private void closeEditor(){if(embeddedDrag!=null){embeddedDrag.cancel();embeddedDrag=null;embeddedDragHandle=null;}SkillVfxWorldPreviewController.clear(this);preview.stop();AbilityVfxLocalPreview.closeEditor();previewOwned=false;previewFrame=null;authoringFrame=null;minecraft.setScreen(parent);}
    ...
    private void ensureEmbeddedPreview(SkillVfxModel.Primitive selected){if(selected==null||!worldPreviewEntryAllowed())return;if(authoringFrame==null)begin3dAuthoring();}
    public boolean embeddedAuthoringActive(){return !displaySettings&&picker==Picker.NONE&&authoringFrame!=null&&primitive(document())!=null;}
    public AbilityVfx.Frame embeddedAuthoringFrame(){return authoringFrame;}
    public SkillVfxDirectAuthoring.MotionHandleTarget embeddedMotionHandleTarget(){return SkillVfxDirectAuthoring.MotionHandleTarget.PHASE;}
    public SkillVfxDirectAuthoring.AuthoringTool embeddedAuthoringTool(){return embeddedTool;}
    public void updateEmbeddedCamera(CameraRenderState cam){if(cam==null||cam.pos==null||cam.orientation==null||cam.projectionMatrix==null)return;Vector3f right=new Vector3f(1,0,0).rotate(cam.orientation),up=new Vector3f(0,1,0).rotate(cam.orientation),forward=new Vector3f(0,0,-1).rotate(cam.orientation);double focal=Math.abs(cam.projectionMatrix.m11())*height/2d;try{embeddedCameraProjection=SkillVfxDirectAuthoring.fromBasis(new SkillVfxDirectAuthoring.Vec(right.x,right.y,right.z),new SkillVfxDirectAuthoring.Vec(up.x,up.y,up.z),new SkillVfxDirectAuthoring.Vec(forward.x,forward.y,forward.z),new SkillVfxDirectAuthoring.Vec(cam.pos.x,cam.pos.y,cam.pos.z),focal);}catch(IllegalArgumentException ignored){embeddedCameraProjection=null;}}
    private boolean insideEmbeddedViewport(double x,double y){var r=layout.bounds(width,height).preview();return embeddedAuthoringActive()&&x>=r.x()&&x<r.x()+r.width()&&y>=r.y()+26&&y<r.y()+r.height();}
    private Optional<SkillVfxDirectAuthoring.Pick> embeddedHit(double x,double y){var selected=primitive(document());if(selected==null||embeddedCameraProjection==null)return Optional.empty();return SkillVfxDirectAuthoring.pick(SkillVfxDirectAuthoring.toolHandles(selected,authoringFrame,authoringAction(),authoringProgress(),embeddedMotionHandleTarget(),embeddedTool),embeddedCameraProjection,x-width/2.0,height/2.0-y,12);}
    ...
    public List<SkillVfxDirectAuthoring.PrimitiveTarget> authoringTargets(){var d=document();if(d==null)return List.of();var out=new ArrayList<SkillVfxDirectAuthoring.PrimitiveTarget>();for(var emission:d.visual().emissions(hook)){SkillVfxModel.GameplayAction action=emission.actionIndex()>=0&&emission.actionIndex()<d.baseline().gameplay().size()?d.baseline().gameplay().get(emission.actionIndex()):null;for(var p:emission.primitives())out.add(new SkillVfxDirectAuthoring.PrimitiveTarget(emission.id(),p,action));}return List.copyOf(out);}
    public void selectAuthoringTarget(SkillVfxDirectAuthoring.PrimitiveTarget target){if(target==null||document()==null)return;emissionId=target.emissionId();primitiveId=target.primitive().id();controlPointOffset=0;document().select("primitive",primitiveId);authoringPreviewChanged(authoringFrame);}
