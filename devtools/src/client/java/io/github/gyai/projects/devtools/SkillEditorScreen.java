...
public final class SkillEditorScreen extends ProjectSThemedScreen {
    ...
    private void buildPicker(AbilityVisualEditorDocument doc){
        ...
        for(var card:page.cards()){
            ...
            if(picker==Picker.PRIMITIVE){var type=SkillVfxModel.PrimitiveType.valueOf(card.item().id());var control=button(card.action().x(),card.action().y(),card.action().width(),"選択",()->choosePrimitive(type));control.setTooltip(Tooltip.create(Component.literal(card.item().tooltip())));add(control);}
            else{var choice=SkillVfxAuthoring.appearances().stream().filter(value->value.appearance().id().equals(card.item().id())).findFirst().orElseThrow();var control=button(card.action().x(),card.action().y(),card.action().width(),"選択",()->chooseAppearance(choice.appearance()));control.setTooltip(Tooltip.create(Component.literal(card.item().tooltip())));add(control);}
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor graphics,int mouseX,int mouseY,float tickProgress){var t=ProjectSThemeManager.get().activeTheme().tokens();graphics.fill(0,0,width,height,t.background());var d=document();int statusX=width<760?240:8,statusY=width<760?10:34;graphics.text(font,d!=null&&d.dirty()?"● 未反映の変更があります":"変更なし",statusX,statusY,t.textMuted(),false);if(!displaySettings&&picker==Picker.NONE){var b=layout.bounds(width,height);panel(graphics,b.tree(),"VFX ツリー");panel(graphics,b.preview(),"ワールド内プレビュー / 操作");panel(graphics,b.inspector(),"詳細");panel(graphics,b.timeline(),"VFX タイムライン");drawTimeline(graphics,d,b.timeline(),t);}else if(displaySettings)graphics.text(font,"表示設定",8,48,t.textPrimary(),false);if(picker!=Picker.NONE)for(var card:pickerCards){var box=card.bounds();graphics.fill(box.x(),box.y(),box.x()+box.width(),box.y()+box.height(),t.surfaceRaised());graphics.outline(box.x(),box.y(),box.width(),box.height(),t.borderCard());}for(var placement:glyphs)drawGlyph(graphics,placement,t);super.extractRenderState(graphics,mouseX,mouseY,tickProgress);for(var label:labels)graphics.text(font,label.text(),label.x(),label.y(),t.textSecondary(),false);}
