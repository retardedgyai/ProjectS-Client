...
public final class SkillVfxWorldPreviewController {
 ...
 private static void render(net.minecraft.client.gui.GuiGraphicsExtractor graphics,net.minecraft.client.DeltaTracker delta){Minecraft mc=Minecraft.getInstance();if(screen==null||mc.player==null||mc.level==null||mc.options.hideGui)return;int x=8,y=8,max=Math.max(1,Math.min(196,graphics.guiWidth()-16));var hud=SkillVfxWorldPreviewHudLayout.layout(max,mc.font::width,"VFX プレビュー",screen.worldPreviewHookLabel()+" / "+screen.worldPreviewPrimitiveLabel()+" / "+screen.worldPreviewAnchorLabel(),key(toggle)+" 再生  "+key(restart)+" 最初から  "+key(returnToEditor)+" 戻る");graphics.fill(x-3,y-3,x+hud.width()+3,y+hud.height(),0xA8000000);for(int index=0;index<hud.lines().size();index++)graphics.text(mc.font,hud.lines().get(index),x,y+index*12,index==0?0xFF55DFFF:index==1?0xFFFFFFFF:0xFFE0E8F0,false);}
