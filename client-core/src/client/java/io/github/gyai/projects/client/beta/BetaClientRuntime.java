...
public final class BetaClientRuntime {
    ...
    private static final EnumSet<BetaProtocol.Capability> ACKNOWLEDGED = EnumSet.complementOf(
            EnumSet.of(BetaProtocol.Capability.MOB_EDITOR_V2));
    ...
    public static Optional<BetaCapabilityAcknowledgementPayload> receive(
        ...
        ELEMENTS.clear();
        return LIFECYCLE.accept(payload.decoded().value(), ACKNOWLEDGED::contains)
    ...
    /** Optional modules opt in before the server advertises capabilities. */
    public static void enableCapability(BetaProtocol.Capability capability) {
        ACKNOWLEDGED.add(capability);
    }

    /** Read-only capability policy view for Core-only and optional-module boundary tests. */
    public static boolean isCapabilityEnabled(BetaProtocol.Capability capability) {
        return ACKNOWLEDGED.contains(capability);
    }
}
